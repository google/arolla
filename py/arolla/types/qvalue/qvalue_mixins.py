...
_invoke_op = rl_abc.invoke_op


def _invoke_binary_op(op, qvalue, other) -> rl_abc.AnyQValue:
  try:
...
def _invoke_binary_rop(op, other, qvalue) -> rl_abc.AnyQValue:
  try:
...
class PresenceQValueMixin:
  ...
  def __eq__(self, other) -> rl_abc.AnyQValue | rl_abc.Expr:  # pytype: disable=signature-mismatch
    if isinstance(other, rl_abc.Expr):
      return NotImplemented  # fallback to Expr.__eq__
    return _invoke_op('core.equal', (self, _as_qvalue(other)))

  def __ne__(self, other) -> rl_abc.AnyQValue | rl_abc.Expr:  # pytype: disable=signature-mismatch
    if isinstance(other, rl_abc.Expr):
...
class FloatingPointArithmeticQValueMixin(IntegralArithmeticQValueMixin):
  ...
  def __truediv__(self, other) -> rl_abc.AnyQValue:
    return _invoke_binary_op('math.divide', self, other)

  def __rtruediv__(self, other) -> rl_abc.AnyQValue:
    return _invoke_binary_rop('math.divide', other, self)

  def __pow__(self, other) -> rl_abc.AnyQValue:
    return _invoke_binary_op('math.pow', self, other)

  def __rpow__(self, other) -> rl_abc.AnyQValue:
