...
"""Tests for arolla.types.qtype.dense_array_qtype."""
...
class ArrayQTypeTest(parameterized.TestCase):
  ...
  @parameterized.parameters(
      (
          array_qtype.array_boolean,
          array_qtype.ARRAY_BOOLEAN,
          scalar_utils_test_helpers.BOOLEAN_TEST_DATA,
      ),
      (
          array_qtype.array_bytes,
          array_qtype.ARRAY_BYTES,
          scalar_utils_test_helpers.BYTES_TEST_DATA,
      ),
      (
          array_qtype.array_float32,
          array_qtype.ARRAY_FLOAT32,
          scalar_utils_test_helpers.FLOAT_TEST_DATA,
      ),
      (
          array_qtype.array_float64,
          array_qtype.ARRAY_FLOAT64,
          scalar_utils_test_helpers.FLOAT_TEST_DATA,
      ),
      (
          array_qtype.array_weak_float,
          array_qtype.ARRAY_WEAK_FLOAT,
          scalar_utils_test_helpers.FLOAT_TEST_DATA,
      ),
      (
          array_qtype.array_int32,
          array_qtype.ARRAY_INT32,
          scalar_utils_test_helpers.INDEX_TEST_DATA,
      ),
      (
          array_qtype.array_int64,
          array_qtype.ARRAY_INT64,
          scalar_utils_test_helpers.INDEX_TEST_DATA,
      ),
      (
          array_qtype.array_uint64,
          array_qtype.ARRAY_UINT64,
          scalar_utils_test_helpers.INDEX_TEST_DATA,
      ),
      (
          array_qtype.array_text,
          array_qtype.ARRAY_TEXT,
          scalar_utils_test_helpers.TEXT_TEST_DATA,
      ),
      (
          array_qtype.array_unit,
          array_qtype.ARRAY_UNIT,
  ...
  def test_array_from_values(self, array_factory, expected_qtype, test_data):
    ...
    self.assertListEqual(
        array_qtype.get_array_py_value(qvalue),
        list(expected_output),
    )

  def test_array_from_values_generator(self):
    self.assertListEqual(
        array_qtype.get_array_py_value(
            array_qtype.array_float32(map(float, range(100)))
        ),
        list(range(100)),
    )

  @parameterized.parameters(
      (array_qtype.array_unit, [0]),
      (array_qtype.array_boolean, [0]),
      (array_qtype.array_boolean, [1]),
      (array_qtype.array_boolean, [[]]),
      (array_qtype.array_bytes, ['str']),
      (array_qtype.array_text, [b'bytes']),
      (array_qtype.array_int32, ['0']),
      (array_qtype.array_int64, ['0']),
      (array_qtype.array_uint64, ['0']),
      (array_qtype.array_float32, ['0']),
      (array_qtype.array_float64, ['0']),
      (array_qtype.array_weak_float, ['0']),
  )
  ...
  @parameterized.parameters(
      (numpy.bool_, array_qtype.array_boolean),
      (numpy.int32, array_qtype.array_int32),
      (numpy.int64, array_qtype.array_int64),
      (numpy.uint64, array_qtype.array_uint64),
      (numpy.float32, array_qtype.array_float32),
      (numpy.float64, array_qtype.array_float64),
      (numpy.float64, array_qtype.array_weak_float),
  )
  ...
  def test_array_from_ids_and_values_value_error(self):
    with self.assertRaisesRegex(
      ...
      array_qtype.array_int64(values=[], ids=[0], size=1)
    with self.assertRaisesRegex(
        ValueError, re.escape('expected arrays of the same sizes')
    ):
      array_qtype.array_int64(values=[0], ids=[], size=1)

  def test_array_from_ids_and_values_type_error(self):
    with self.assertRaisesRegex(
        TypeError, re.escape("'str' object cannot be interpreted as an integer")
    ):
      array_qtype.array_int64(values=[], ids=[''], size=1)
  ...
  def test_array_boolean_from_size_and_value(self, value, expected_output):
    qvalue = array_qtype.array_boolean(value, size=5)
    self.assertEqual(qvalue.qtype, array_qtype.ARRAY_BOOLEAN)
    self.assertListEqual(
        array_qtype.get_array_py_value(qvalue),
        [expected_output] * 5,
    )
  ...
  def test_array_bytes_from_size_and_value(self, value, expected_output):
    qvalue = array_qtype.array_bytes(value, size=5)
    self.assertEqual(qvalue.qtype, array_qtype.ARRAY_BYTES)
    self.assertListEqual(
        array_qtype.get_array_py_value(qvalue),
        [expected_output] * 5,
    )
  ...
  def test_array_float32_from_size_and_value(self, value, expected_output):
    qvalue = array_qtype.array_float32(value, size=5)
    self.assertEqual(qvalue.qtype, array_qtype.ARRAY_FLOAT32)
    self.assertListEqual(
        array_qtype.get_array_py_value(qvalue),
        [expected_output] * 5,
  ...
  def test_array_float64_from_size_and_value(self, value, expected_output):
    qvalue = array_qtype.array_float64(value, size=5)
    self.assertEqual(qvalue.qtype, array_qtype.ARRAY_FLOAT64)
    self.assertListEqual(
        array_qtype.get_array_py_value(qvalue),
        [expected_output] * 5,
    )
  ...
  def test_array_weak_float_from_size_and_value(self, value, expected_output):
    qvalue = array_qtype.array_weak_float(value, size=5)
    self.assertEqual(qvalue.qtype, array_qtype.ARRAY_WEAK_FLOAT)
    self.assertListEqual(
        array_qtype.get_array_py_value(qvalue),
        [expected_output] * 5,
    )
  ...
  def test_array_int32_from_size_and_value(self, value, expected_output):
    qvalue = array_qtype.array_int32(value, size=5)
    self.assertEqual(qvalue.qtype, array_qtype.ARRAY_INT32)
    self.assertListEqual(
        array_qtype.get_array_py_value(qvalue),
        [expected_output] * 5,
    )
  ...
  def test_array_int64_from_size_and_value(self, value, expected_output):
    qvalue = array_qtype.array_int64(value, size=5)
    self.assertEqual(qvalue.qtype, array_qtype.ARRAY_INT64)
    self.assertListEqual(
        array_qtype.get_array_py_value(qvalue),
        [expected_output] * 5,
    )
  ...
  def test_array_uint64_from_size_and_value(self, value, expected_output):
    qvalue = array_qtype.array_uint64(value, size=5)
    self.assertEqual(qvalue.qtype, array_qtype.ARRAY_UINT64)
    self.assertListEqual(
        array_qtype.get_array_py_value(qvalue),
        [expected_output] * 5,
    )
  ...
  def test_array_text_from_size_and_value(self, value, expected_output):
    qvalue = array_qtype.array_text(value, size=5)
    self.assertEqual(qvalue.qtype, array_qtype.ARRAY_TEXT)
    self.assertListEqual(
        array_qtype.get_array_py_value(qvalue),
        [expected_output] * 5,
    )
  ...
  def test_array_unit_from_size_and_value(self, value, expected_output):
    qvalue = array_qtype.array_unit(value, size=5)
    self.assertEqual(qvalue.qtype, array_qtype.ARRAY_UNIT)
    self.assertListEqual(
        array_qtype.get_array_py_value(qvalue),
        [expected_output] * 5,
    )
  ...
  @parameterized.parameters(
      *((qtype, True) for qtype in array_qtype.ARRAY_QTYPES),
      *((qtype, True) for qtype in dense_array_qtype.DENSE_ARRAY_QTYPES),
      (scalar_qtype.INT64, False),
  )
  def test_is_array_qtype(self, qtype, expected_value):
    self.assertEqual(array_qtype.is_array_qtype(qtype), expected_value)

  def test_is_array_qtype_type_error(self):
    with self.assertRaises(TypeError):
      array_qtype.is_array_qtype(0)  # pytype: disable=wrong-arg-types

  def test_make_array_qtype(self):
    self.assertEqual(
        array_qtype.make_array_qtype(scalar_qtype.INT32),
        array_qtype.ARRAY_INT32,
    )
    self.assertEqual(
        array_qtype.make_array_qtype(optional_qtype.OPTIONAL_INT32),
        array_qtype.ARRAY_INT32,
    )
    with self.assertRaises(TypeError):
      _ = array_qtype.make_array_qtype(int)  # pytype: disable=wrong-arg-types
    with self.assertRaises(casting.QTypeError):
      _ = (array_qtype.make_array_qtype(array_qtype.ARRAY_INT32),)

  def test_array_from_size_and_value_missing_size(self):
    with self.assertRaisesWithLiteralMatch(
        TypeError, 'missing parameter: size'
    ):
      array_qtype.array_unit(values=[], ids=[])

  @parameterized.parameters(
      # arrays
      (
          array_qtype.array_boolean,
          optional_qtype.optional_boolean,
          scalar_utils_test_helpers.BOOLEAN_TEST_DATA,
      ),
      (
          array_qtype.array_bytes,
          optional_qtype.optional_bytes,
          scalar_utils_test_helpers.BYTES_TEST_DATA,
      ),
      (
          array_qtype.array_float32,
          optional_qtype.optional_float32,
          scalar_utils_test_helpers.FLOAT_TEST_DATA,
      ),
      (
          array_qtype.array_float64,
          optional_qtype.optional_float64,
          scalar_utils_test_helpers.FLOAT_TEST_DATA,
      ),
      (
          array_qtype.array_weak_float,
          optional_qtype.optional_weak_float,
          scalar_utils_test_helpers.FLOAT_TEST_DATA,
      ),
      (
          array_qtype.array_int32,
          optional_qtype.optional_int32,
          scalar_utils_test_helpers.INDEX_TEST_DATA,
      ),
      (
          array_qtype.array_int64,
          optional_qtype.optional_int64,
          scalar_utils_test_helpers.INDEX_TEST_DATA,
      ),
      (
          array_qtype.array_uint64,
          optional_qtype.optional_uint64,
          scalar_utils_test_helpers.INDEX_TEST_DATA,
      ),
      (
          array_qtype.array_text,
          optional_qtype.optional_text,
          scalar_utils_test_helpers.TEXT_TEST_DATA,
      ),
      (
          array_qtype.array_unit,
          optional_qtype.optional_unit,
          scalar_utils_test_helpers.UNIT_TEST_DATA,
      ),
      # dense_arrays
      (
          dense_array_qtype.dense_array_boolean,
          optional_qtype.optional_boolean,
          scalar_utils_test_helpers.BOOLEAN_TEST_DATA,
      ),
      (
          dense_array_qtype.dense_array_bytes,
          optional_qtype.optional_bytes,
          scalar_utils_test_helpers.BYTES_TEST_DATA,
      ),
      (
          dense_array_qtype.dense_array_float32,
          optional_qtype.optional_float32,
          scalar_utils_test_helpers.FLOAT_TEST_DATA,
      ),
      (
          dense_array_qtype.dense_array_float64,
          optional_qtype.optional_float64,
          scalar_utils_test_helpers.FLOAT_TEST_DATA,
      ),
      (
          dense_array_qtype.dense_array_weak_float,
          optional_qtype.optional_weak_float,
          scalar_utils_test_helpers.FLOAT_TEST_DATA,
      ),
      (
          dense_array_qtype.dense_array_int32,
          optional_qtype.optional_int32,
          scalar_utils_test_helpers.INDEX_TEST_DATA,
      ),
      (
          dense_array_qtype.dense_array_int64,
          optional_qtype.optional_int64,
          scalar_utils_test_helpers.INDEX_TEST_DATA,
      ),
      (
          dense_array_qtype.dense_array_uint64,
          optional_qtype.optional_uint64,
          scalar_utils_test_helpers.INDEX_TEST_DATA,
      ),
      (
          dense_array_qtype.dense_array_text,
          optional_qtype.optional_text,
          scalar_utils_test_helpers.TEXT_TEST_DATA,
      ),
      (
          dense_array_qtype.dense_array_unit,
          optional_qtype.optional_unit,
          scalar_utils_test_helpers.UNIT_TEST_DATA,
      ),
  )
  def test_get_array_item(self, array_factory, item_factory, test_data):
    ...
    for i in range(-len(expected_output), len(expected_output)):
      actual_item = array_qtype.get_array_item(array, i)
    ...
    with self.assertRaises(IndexError):
      array_qtype.get_array_item(array, -len(expected_output) - 1)
    with self.assertRaises(IndexError):
      array_qtype.get_array_item(array, len(expected_output))

  def test_get_array_item_type_error(self):
    with self.assertRaisesWithLiteralMatch(
        TypeError, "missing 2 required positional arguments: 'array', 'i'"
    ):
      array_qtype.get_array_item()  # pytype: disable=missing-parameter
    with self.assertRaisesWithLiteralMatch(
        TypeError, "missing 1 required positional argument: 'i'"
    ):
      array_qtype.get_array_item(object())  # pytype: disable=missing-parameter
    with self.assertRaisesWithLiteralMatch(
        TypeError, 'expected 2 positional arguments, but 3 were given'
    ):
      array_qtype.get_array_item(object(), object(), object())  # pytype: disable=wrong-arg-count
    with self.assertRaisesWithLiteralMatch(
        TypeError, 'expected an array, got object'
    ):
      array_qtype.get_array_item(object(), 0)  # pytype: disable=wrong-arg-types
    with self.assertRaisesWithLiteralMatch(
        TypeError, 'expected an array, got arolla.abc.qtype.QType'
    ):
      array_qtype.get_array_item(arolla_abc.QTYPE, 0)
    with self.assertRaisesWithLiteralMatch(
        TypeError, "'object' object cannot be interpreted as an integer"
    ):
      array_qtype.get_array_item(
          array_qtype.array_unit([]), object()  # pytype: disable=wrong-arg-types
      )
    with self.assertRaisesWithLiteralMatch(
        TypeError, "'NoneType' object cannot be interpreted as an integer"
    ):
      array_qtype.get_array_item(
          array_qtype.array_unit([]), None  # pytype: disable=wrong-arg-types
      )
  ...
  def test_get_array_item_missing_optional_error(self):
    with self.assertRaises(scalar_utils.MissingOptionalError):
      array_qtype.get_array_item(
          array_qtype.array_unit([]),
          optional_qtype.optional_int32(None),
      )
  ...
  def test_get_array_py_value(self):
    with self.assertRaisesWithLiteralMatch(
      ...
      array_qtype.get_array_py_value(arolla_abc.QTYPE)

  @parameterized.parameters(
      (array_qtype.array_unit, [[]]),
      (dense_array_qtype.dense_array_unit, [[]]),
      (array_qtype.array_unit, [[None], [True]]),
      (dense_array_qtype.dense_array_unit, [[None], [True]]),
      (array_qtype.array_unit, itertools.repeat([True], 10240)),
      (dense_array_qtype.dense_array_unit, itertools.repeat([True], 10240)),
      (array_qtype.array_float32, [[]]),
      (dense_array_qtype.dense_array_float32, [[]]),
      (array_qtype.array_float32, [[None], [1.5]]),
      (dense_array_qtype.dense_array_float32, [[]]),
      (array_qtype.array_float32, [[None], [1.5]]),
      (dense_array_qtype.dense_array_float32, [[None], [1.5]]),
      (array_qtype.array_float32, itertools.repeat([1.5], 10240)),
      (dense_array_qtype.dense_array_float32, itertools.repeat([1.5], 10240)),
  )
  def test_array_concat(self, array_factories, valueses):
    valueses = tuple(valueses)
    actual_qvalue = array_qtype.array_concat(*map(array_factories, valueses))
