...
class DictGetRowTest(parameterized.TestCase, backend_test_base.SelfEvalMixin):
  ...
  def test_qtype_signatures(self):
    ...
    arolla.testing.assert_qtype_signatures(
        M.dict._get_row, QTYPE_SIGNATURES, possible_qtypes=possible_qtypes  # pyrefly: ignore[missing-attribute]
    )
  ...
  def test_eval(self, dict_qvalue, keys_to_query, values_to_expect):
    actual_value = self.eval(M.dict._get_row(dict_qvalue, keys_to_query))  # pyrefly: ignore[missing-attribute]
    arolla.testing.assert_qvalue_allequal(actual_value, values_to_expect)

  def test_error_dtype_downcasting_not_allowed(self):
    self.require_self_eval_is_called = False
    with self.assertRaisesRegex(
      ...
      M.dict._get_row(  # pyrefly: ignore[missing-attribute]
          M.annotation.qtype(  # pyrefly: ignore[not-callable]
              L.d, arolla.types.make_key_to_row_dict_qtype(arolla.INT32)
          ),
          M.annotation.qtype(L.key, arolla.INT64),  # pyrefly: ignore[not-callable]
      )
