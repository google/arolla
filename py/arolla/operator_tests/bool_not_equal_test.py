...
"""Tests for M.bool.not_equal operator."""
...
def gen_test_data():
  """Yields test data for bool.not_equal operator.
...
def gen_qtype_signatures():
  """Yields qtype signatures for bool.not_equal.
...
class BoolNotEqualTest(parameterized.TestCase, backend_test_base.SelfEvalMixin):
  ...
  def test_qtype_signatures(self):
    ...
    arolla.testing.assert_qtype_signatures(M.bool.not_equal, QTYPE_SIGNATURES)  # pyrefly: ignore[missing-attribute]

  @parameterized.parameters(
      pointwise_test_utils.gen_cases(TEST_DATA, *QTYPE_SIGNATURES)
  )
  def test_eval(self, lhs, rhs, expected_value):
    actual_value = self.eval(M.bool.not_equal(lhs, rhs))  # pyrefly: ignore[missing-attribute]
