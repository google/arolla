...
def get_test_data():
  ...
  for child_qtype in arolla.types.INTEGRAL_QTYPES:
    for parent_qtype in arolla.types.INTEGRAL_QTYPES:
      if (
          arolla.eval(M.qtype.common_qtype(child_qtype, parent_qtype))  # pyrefly: ignore[missing-attribute]
          == parent_qtype
      ):
...
def gen_test_cases_tuples(test_data):
  for example in test_data:
    for _, array_fn in utils.ARRAY_FACTORIES:
      ...
      # defined separately.
      child_keys = M.core.make_tuple(  # pyrefly: ignore[missing-attribute]
          array_fn(example.child_keys, value_qtype=example.child_qtype),
          array_fn(example.child_keys, value_qtype=example.child_qtype),
          array_fn([0] * len(example.child_keys), value_qtype=arolla.INT64),
      )
      parent_keys = M.core.make_tuple(  # pyrefly: ignore[missing-attribute]
          array_fn(example.parent_keys, value_qtype=example.parent_qtype),
          array_fn(example.parent_keys, value_qtype=example.parent_qtype),
...
def get_qtype_signature(child_keys, parent_keys):
  return (
      ...
      if arolla.eval(M.qtype.is_dense_array_qtype(child_keys.qtype))  # pyrefly: ignore[missing-attribute]
...
class EdgeFromKeysTest(parameterized.TestCase, backend_test_base.SelfEvalMixin):
  ...
  def test_qtype_signatures(self):
    ...
    arolla.testing.assert_qtype_signatures(
        M.edge.from_keys, QTYPE_SIGNATURES, possible_qtypes=_ALL_POSSIBLE_QTYPES  # pyrefly: ignore[missing-attribute]
    )

  @parameterized.parameters(*(TEST_CASES + TEST_CASES_TUPLES))
  def test_simple(self, child_keys, parent_keys, mapping, mapping_size):
    result = self.eval(M.edge.from_keys(child_keys, parent_keys))  # pyrefly: ignore[missing-attribute]
    arolla.testing.assert_qvalue_allequal(
        arolla.eval(M.edge.mapping(result)), mapping  # pyrefly: ignore[missing-attribute]
    )
  ...
  def test_with_missing_child_keys(self, array_factory):
    ...
    result = self.eval(M.edge.from_keys(child_keys, parent_keys))  # pyrefly: ignore[missing-attribute]
    arolla.testing.assert_qvalue_allequal(
        arolla.eval(M.edge.mapping(result)),  # pyrefly: ignore[missing-attribute]
  ...
  def test_duplicates_in_parent_keys(self, array_factory):
    ...
    with self.assertRaisesRegex(ValueError, 'duplicated key'):
      _ = self.eval(M.edge.from_keys(child_keys, parent_keys))  # pyrefly: ignore[missing-attribute]
  ...
  def test_tuples(self, array_factory):
    ...
    self.assertEqual(
        result.parent_size, arolla.eval(M.core.get_first(parent_keys)).size  # pyrefly: ignore[missing-attribute]
    )
