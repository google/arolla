...
def agg_into_scalar(x):
  ...
  initial = utils.optional(0, arolla.types.get_scalar_qtype(x.qtype))
  values = arolla.abc.invoke_op("array.present_values", (x,))
  if values.size == 0:
    return utils.optional(None, arolla.types.get_scalar_qtype(x.qtype))
...
class MathSumSparseTest(
  ...
  def test_qtype_signatures(self):
    ...
    self.assertCountEqual(
        pointwise_test_utils.detect_qtype_signatures(M.math._sum_sparse),
        QTYPE_SIGNATURES,
    )
