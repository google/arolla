...
class MathFloorDivTest(parameterized.TestCase, backend_test_base.SelfEvalMixin):
  ...
  def testQTypeSignatures(self):
    ...
    arolla.testing.assert_qtype_signatures(M.math.floordiv, QTYPE_SIGNATURES)  # pyrefly: ignore[missing-attribute]

  @parameterized.parameters(
      pointwise_test_utils.gen_cases(TEST_DATA, *QTYPE_SIGNATURES)
  )
  def testValue(self, arg_1, arg_2, expected_result):
    result = self.eval(M.math.floordiv(arg_1, arg_2))  # pyrefly: ignore[missing-attribute]
