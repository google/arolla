...
class QTypeGetKeyToRowDictKeyQTypeTest(parameterized.TestCase):
  ...
  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        M.qtype._get_key_to_row_dict_key_qtype, QTYPE_SIGNATURES  # pyrefly: ignore[missing-attribute]
    )
  ...
  def test_eval(self, arg, expected_value):
    actual_value = arolla.eval(M.qtype._get_key_to_row_dict_key_qtype(arg))  # pyrefly: ignore[missing-attribute]
