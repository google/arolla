...
"""Tests for M.math.subtract operator."""
...
def gen_test_data():
  """Yields test data for math.subtract operator.
  ...
  yield from (
      (arolla.int32(-(2**31)), arolla.int32(1), arolla.int32(2**31 - 1)),
      (arolla.int64(-(2**63)), arolla.int64(1), arolla.int64(2**63 - 1)),
      (arolla.int32(2**31 - 1), arolla.int32(-1), arolla.int32(-(2**31))),
      (arolla.int64(2**63 - 1), arolla.int64(-1), arolla.int64(-(2**63))),
  )


def gen_qtype_signatures():
  """Yields qtype signatures for math.subtract.
...
class MathSubtractTest(parameterized.TestCase, backend_test_base.SelfEvalMixin):

  def testQTypeSignatures(self):
    self.require_self_eval_is_called = False
    arolla.testing.assert_qtype_signatures(M.math.subtract, QTYPE_SIGNATURES)  # pyrefly: ignore[missing-attribute]

  @parameterized.parameters(
      pointwise_test_utils.gen_cases(TEST_DATA, *QTYPE_SIGNATURES)
  )
  def testValue(self, arg_1, arg_2, expected_result):
    result = self.eval(M.math.subtract(arg_1, arg_2))  # pyrefly: ignore[missing-attribute]
