...
class EdgePairLeftTest(parameterized.TestCase, backend_test_base.SelfEvalMixin):
  ...
  def test_overflow_in_cpp_detected(self, array_factory):
    ...
    with self.subTest('edge.pair_left s*s overflow'):
      # A single group with large size so that s*s overflows int64.
      s = int64_max // 2
      with self.assertRaisesRegex(ValueError, 'overflow'):
        self.eval(M.edge.pair_left(array_factory([s], arolla.INT64)))

    with self.subTest('edge.pair_left accumulation overflow'):
      ...
      s = 2_500_000_000
      with self.assertRaisesRegex(ValueError, 'overflow'):
        self.eval(M.edge.pair_left(array_factory([s, s], arolla.INT64)))
