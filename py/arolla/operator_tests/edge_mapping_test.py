...
def gen_test_cases():
  ...
  for mapping in mapping_values:
    for _, array_fn in utils.ARRAY_FACTORIES:
      ...
      yield M.edge.from_mapping(array, parent_size=7), array  # pyrefly: ignore[missing-attribute]
      yield M.edge.to_scalar(array), array_fn([0] * len(mapping), arolla.INT64)  # pyrefly: ignore[missing-attribute]
  split_points_data = (([0], []), ([0, 2, 2, 5], [0, 0, 2, 2, 2]))
  for split_points, mapping in split_points_data:
    for _, array_fn in utils.ARRAY_FACTORIES:
      yield M.edge.from_split_points(  # pyrefly: ignore[missing-attribute]
...
QTYPE_SIGNATURES = (
    (arolla.eval(M.qtype.qtype_of(edge)), arolla.eval(M.qtype.qtype_of(array)))  # pyrefly: ignore[missing-attribute]
...
class EdgeMappingTest(parameterized.TestCase, backend_test_base.SelfEvalMixin):
  ...
  def test_qtype_signatures(self):
    ...
    arolla.testing.assert_qtype_signatures(M.edge.mapping, QTYPE_SIGNATURES)  # pyrefly: ignore[missing-attribute]

  @parameterized.parameters(*TEST_CASES)
  def test_simple(self, edge, mapping):
    result = self.eval(M.edge.mapping(edge))  # pyrefly: ignore[missing-attribute]
