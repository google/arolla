...
"""Tests for M.bitwise.bitwise_and operator."""
...
def gen_test_data():
  """Yields test data for bitwise.bitwise_and operator.
...
def gen_qtype_signatures():
  """Yields qtype signatures for bitwise.bitwise_and.

  Yields: (arg_1_qtype, arg_2_qtype, result_qtype)
  """
  arg_qtypes = pointwise_test_utils.lift_qtypes(*arolla.types.INTEGRAL_QTYPES)
...
class BitwiseBitwiseAndTest(parameterized.TestCase):

  def testQTypeSignatures(self):
    self.assertEqual(
        frozenset(QTYPE_SIGNATURES),
        frozenset(
            pointwise_test_utils.detect_qtype_signatures(M.bitwise.bitwise_and)
        ),
    )
  ...
  def testValue(self, lhs, rhs, expected_value):
    actual_value = arolla.eval(M.bitwise.bitwise_and(lhs, rhs))
