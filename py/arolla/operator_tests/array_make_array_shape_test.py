...
class ArrayMakeArrayShapeTest(
  ...
  def test_qtype_signatures(self):
    ...
    arolla.testing.assert_qtype_signatures(
        M.array.make_array_shape, QTYPE_SIGNATURES  # pyrefly: ignore[missing-attribute]
    )

  @parameterized.parameters(*TEST_CASES)
  def test_value(self, arg, expected_result):
    result = self.eval(M.array.make_array_shape(arg))  # pyrefly: ignore[missing-attribute]
  ...
  def test_negative_size_error(self):
    ...
    with self.assertRaises(ValueError):
      self.eval(M.array.make_array_shape(-1))  # pyrefly: ignore[missing-attribute]
