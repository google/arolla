...
class MathLogSigmoidTest(parameterized.TestCase):
  ...
  def testQTypeSignatures(self):
    arolla.testing.assert_qtype_signatures(M.math.log_sigmoid, QTYPE_SIGNATURES)  # pyrefly: ignore[missing-attribute]

  @parameterized.parameters(
      pointwise_test_utils.gen_cases(TEST_DATA, *QTYPE_SIGNATURES)
  )
  def testValue(self, arg, expected_value):
    actual_value = arolla.eval(M.math.log_sigmoid(arg))  # pyrefly: ignore[missing-attribute]
