...
"""Tests for M.math.minimum operator."""
...
def gen_test_data():
  """Yields test data for math.minimum operator.

  Yields: (arg_1, arg_2, result)
  """
  values = (None, NAN, -INF, INF, -1, 0, 1, 1.5)
  for arg_1, arg_2 in itertools.product(values, repeat=2):
    yield (arg_1, arg_2, reference_minimum(arg_1, arg_2))


def gen_qtype_signatures():
  """Yields qtype signatures for math.minimum.
...
class MathMinimumTest(parameterized.TestCase, backend_test_base.SelfEvalMixin):

  def testQTypeSignatures(self):
    self.require_self_eval_is_called = False
    arolla.testing.assert_qtype_signatures(M.math.minimum, QTYPE_SIGNATURES)  # pyrefly: ignore[missing-attribute]

  @parameterized.parameters(
      pointwise_test_utils.gen_cases(TEST_DATA, *QTYPE_SIGNATURES)
  )
  def testValue(self, arg_1, arg_2, expected_result):
    result = self.eval(M.math.minimum(arg_1, arg_2))  # pyrefly: ignore[missing-attribute]
