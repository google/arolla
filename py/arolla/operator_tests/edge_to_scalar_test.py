...
class EdgeToScalar(parameterized.TestCase):
  ...
  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(M.edge.to_scalar, QTYPE_SIGNATURES)  # pyrefly: ignore[missing-attribute]

  def test_scalar_to_scalar_edge(self):
    with self.subTest('scalar'):
      output_qvalue = arolla.eval(M.edge.to_scalar(arolla.int32(1)))  # pyrefly: ignore[missing-attribute]
      self.assertEqual(output_qvalue.qtype, arolla.types.SCALAR_TO_SCALAR_EDGE)
    with self.subTest('optional_scalar'):
      output_qvalue = arolla.eval(M.edge.to_scalar(arolla.optional_int32(1)))  # pyrefly: ignore[missing-attribute]
      self.assertEqual(output_qvalue.qtype, arolla.types.SCALAR_TO_SCALAR_EDGE)
  ...
  def test_array_to_scalar_edge(self, n):
    with self.subTest('dense_array'):
      ...
      output_qvalue = arolla.eval(M.edge.to_scalar(input_qvalue))  # pyrefly: ignore[missing-attribute]
      self.assertEqual(
      ...
      arolla.testing.assert_qvalue_equal_by_fingerprint(
          arolla.eval(M.edge.child_shape(output_qvalue)),  # pyrefly: ignore[missing-attribute]
