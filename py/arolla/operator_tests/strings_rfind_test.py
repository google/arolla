...
"""Tests for M.strings.rfind operator."""
...
class StringsRFindTest(parameterized.TestCase, backend_test_base.SelfEvalMixin):
  ...
  def testQTypeSignatures(self):
    ...
    arolla.testing.assert_qtype_signatures(M.strings.rfind, QTYPE_SIGNATURES)  # pyrefly: ignore[missing-attribute]
  ...
  def testValue(self, *test_case):
    *args, expected = test_case
    actual = self.eval(M.strings.rfind(*args))  # pyrefly: ignore[missing-attribute]
  ...
  def testTextBytes(self):
    ...
    with self.assertRaisesRegex(
      ...
      _ = M.strings.rfind('text', b'bytes')  # pyrefly: ignore[missing-attribute]
