...
"""Tests for M.bool.equal operator."""
...
def gen_test_data():
  """Yields test data for bool.equal operator.
...
def gen_qtype_signatures():
  """Yields qtype signatures for bool.equal.
...
class BoolEqualTest(parameterized.TestCase, backend_test_base.SelfEvalMixin):

  def testQTypeSignatures(self):
    self.require_self_eval_is_called = False
    self.assertEqual(
        frozenset(QTYPE_SIGNATURES),
        frozenset(pointwise_test_utils.detect_qtype_signatures(M.bool.equal)),
    )
