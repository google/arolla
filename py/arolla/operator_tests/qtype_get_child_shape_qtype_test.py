...
"""Tests for M.qtype.get_child_shape_qtype operator."""
...
def gen_test_data():
  ...
  yield from (
      ...
      (arolla.DENSE_ARRAY_TO_SCALAR_EDGE, arolla.DENSE_ARRAY_SHAPE),
      (arolla.ARRAY_EDGE, arolla.ARRAY_SHAPE),
      (arolla.ARRAY_TO_SCALAR_EDGE, arolla.ARRAY_SHAPE),
  )
...
class QTypeGetChildShapeQTypeTest(parameterized.TestCase):

  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        M.qtype.get_child_shape_qtype, QTYPE_SIGNATURES  # pyrefly: ignore[missing-attribute]
    )
  ...
  def test_eval(self, arg, expected_value):
    actual_value = arolla.eval(M.qtype.get_child_shape_qtype(arg))  # pyrefly: ignore[missing-attribute]
