...
def gen_test_data():
  """Yields test data for math.mod operator."""
  ...
  yield from (
      (arolla.int32(-(2**31)), IntOnly(-1), 0),
      (arolla.int64(-(2**63)), IntOnly(-1), 0),
  )


def gen_qtype_signatures():
  """Yields qtype signatures for math.mod.
...
class MathModTest(parameterized.TestCase, backend_test_base.SelfEvalMixin):

  def testQTypeSignatures(self):
    self.require_self_eval_is_called = False
    arolla.testing.assert_qtype_signatures(M.math.mod, QTYPE_SIGNATURES)  # pyrefly: ignore[missing-attribute]

  @parameterized.parameters(
      pointwise_test_utils.gen_cases(TEST_DATA, *QTYPE_SIGNATURES)
  )
  def testValue(self, arg_1, arg_2, expected_result):
    result = self.eval(M.math.mod(arg_1, arg_2))  # pyrefly: ignore[missing-attribute]
