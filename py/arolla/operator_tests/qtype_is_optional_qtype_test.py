...
"""Tests for M.qtype.is_optional_qtype operator."""
...
TEST_CASES = (
    ...
    (arolla.OPTIONAL_UNIT, arolla.present()),
    (arolla.OPTIONAL_BOOLEAN, arolla.present()),
    (arolla.OPTIONAL_INT32, arolla.present()),
    (arolla.OPTIONAL_FLOAT32, arolla.present()),
...
class QTypeIsOptionalQTypeTest(parameterized.TestCase):

  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        M.qtype.is_optional_qtype, QTYPE_SIGNATURES  # pyrefly: ignore[missing-attribute]
    )

  @parameterized.parameters(*TEST_CASES)
  def test_eval(self, arg, expected_value):
    actual_value = arolla.eval(M.qtype.is_optional_qtype(arg))  # pyrefly: ignore[missing-attribute]
