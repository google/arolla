...
"""Tests for M.qtype.get_edge_qtype operator."""
...
def gen_test_data():
  ...
  yield from ((x, arolla.NOTHING) for x in non_array_qtypes)
  yield from (
      (x, arolla.DENSE_ARRAY_EDGE) for x in arolla.types.DENSE_ARRAY_QTYPES
  )
  yield from ((x, arolla.ARRAY_EDGE) for x in arolla.types.ARRAY_QTYPES)
...
class QTypeGetEdgeQTypeTest(parameterized.TestCase):

  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        M.qtype.get_edge_qtype, QTYPE_SIGNATURES  # pyrefly: ignore[missing-attribute]
    )
  ...
  def test_eval(self, arg, expected_value):
    actual_value = arolla.eval(M.qtype.get_edge_qtype(arg))  # pyrefly: ignore[missing-attribute]
