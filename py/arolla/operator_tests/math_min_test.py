...
def agg_into_scalar(x):
  ...
  return utils.optional(numpy.min(values), x.value_qtype)
...
class MathMinTest(parameterized.TestCase, backend_test_base.SelfEvalMixin):

  def test_qtype_signatures(self):
    self.require_self_eval_is_called = False
    arolla.testing.assert_qtype_signatures(M.math.min, QTYPE_SIGNATURES)  # pyrefly: ignore[missing-attribute]
  ...
  def test_eval(self, *test_case):
    ...
    arolla.testing.assert_qvalue_allclose(
        self.eval(M.math.min(*args)), expected_result  # pyrefly: ignore[missing-attribute]
    )
