...
class QTypeGetScalarQTypeTest(parameterized.TestCase):
  ...
  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        M.qtype.get_scalar_qtype, QTYPE_SIGNATURES  # pyrefly: ignore[missing-attribute]
    )

  @parameterized.parameters(*TEST_CASES)
  def test_eval(self, arg, expected_value):
    actual_value = arolla.eval(M.qtype.get_scalar_qtype(arg))  # pyrefly: ignore[missing-attribute]
