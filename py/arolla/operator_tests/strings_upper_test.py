...
class StringsUpperTest(parameterized.TestCase, backend_test_base.SelfEvalMixin):
  ...
  def test_qtype_signatures(self):
    ...
    arolla.testing.assert_qtype_signatures(M.strings.upper, QTYPE_SIGNATURES)  # pyrefly: ignore[missing-attribute]

  @parameterized.parameters(
      pointwise_test_utils.gen_cases(TEST_DATA, *QTYPE_SIGNATURES)
  )
  def test_eval(self, arg, expected_value):
    actual_value = self.eval(M.strings.upper(arg))  # pyrefly: ignore[missing-attribute]
