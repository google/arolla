...
absl::StatusOr<size_t> SizeOfSequences(
  ...
  const size_t seq_size = frame.Get(
    input_slots[0].UnsafeToSlot<Sequence>()).size();
  for (size_t i = 1; i < input_slots.size(); ++i) {
    const auto& cur_seq = frame.Get(
      input_slots[i].UnsafeToSlot<Sequence>());
    if (cur_seq.size() != seq_size) {
      return absl::InvalidArgumentError(absl::StrFormat(
          "all sequences should have equal sizes, %d != %d",
 ...
 public:
  ...
  absl::StatusOr<std::unique_ptr<BoundOperator>> DoBind(
    ...
    return MakeBoundOperator([
      ...
      start = std::min(std::max(static_cast<int64_t>(0), start), sequence_size);
      stop = std::min(std::max(start, stop), sequence_size);

      frame.Set(output_slot,
                sequence.subsequence(start, stop - start));
    });
  }
};
  ...
  absl::StatusOr<std::unique_ptr<BoundOperator>> DoBind(
    ...
    return MakeBoundOperator([
      ...
      output_slot = output_slot.UnsafeToSlot<Sequence>()
          ...
          const QTypePtr output_tuple_qtype = TupleQTypeOfInputValueQTypes(
            input_qtypes);
          ...
          for (size_t seq_id = 0; seq_id < seq_size; ++seq_id) {
            ...
            for (size_t tuple_id = 0; tuple_id < args_count; ++tuple_id) {
              const auto& cur_seq = frame.Get(
                input_slots[tuple_id].UnsafeToSlot<Sequence>());
