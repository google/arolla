...
bool IsAlwaysPresentType(const ExprNode& expr) {
  return IsScalarQType(expr.qtype());
}

bool IsAlwaysPresentOptionalValue(const ExprNode& expr) {
...
bool IsAlwaysAbsentOptionalValue(const ExprNode& expr) {
  const auto& optional_qvalue = expr.qvalue();
  return optional_qvalue.has_value() &&
         IsOptionalQType(optional_qvalue->GetType()) &&
         !UnsafeIsPresent(optional_qvalue->AsRef());
...
bool IsLiteral(const ExprNode& node) { return node.is_literal(); }

bool IsOptionalLikeNode(const ExprNode& node) {
...
absl::Status HasRemovalOptimizations(PeepholeOptimizationPack& optimizations) {
  ...
  }
  {
    ...
    ASSIGN_OR_RETURN(
        ...
        CreatePatternBasedOptimization(from, to, {{"a", IsAlwaysPresentType}}));
  }
...
absl::Status PresenceAndRemovalOptimizations(
  ...
  ExprNodePtr b = Placeholder("b");
  {
    ...
    ASSIGN_OR_RETURN(
        optimizations.emplace_back(),
        CreatePatternBasedOptimization(from, a, {{"b", IsAlwaysPresentType}}));
  }
  {
    ASSIGN_OR_RETURN(
        ExprNodePtr from,
    ...
    ASSIGN_OR_RETURN(
        optimizations.emplace_back(),
        CreatePatternBasedOptimization(from, to, {{"a", IsAlwaysPresent}}));
  }
  {
    ASSIGN_OR_RETURN(ExprNodePtr from,
                     CallOpReference("core.presence_and", {a, b}));
...
absl::Status PresenceOrRemovalOptimizations(
  ...
  ASSIGN_OR_RETURN(
      optimizations.emplace_back(),
      CreatePatternBasedOptimization(from, a, {{"a", IsAlwaysPresentType}}));
  return absl::OkStatus();
}
...
absl::Status HasPropagationOptimizations(
  ...
  auto is_literal_or_presence = [](const ExprNode& expr) {
...
absl::Status ToOptionalPropagationOptimizations(
  ...
  }
  {
    ...
    ASSIGN_OR_RETURN(
        optimizations.emplace_back(),
        CreatePatternBasedOptimization(from, to, {{"a", IsLiteral}}));
    ASSIGN_OR_RETURN(
        optimizations.emplace_back(),
        CreatePatternBasedOptimization(from, to, {{"b", IsLiteral}}));
  }
  return absl::OkStatus();
}
...
absl::Status WhereOptimizations(PeepholeOptimizationPack& optimizations) {
  ...
  {  // (P.a & P.c) | (P.b & ~P.c)  -> where(P.c, P.a, P.b)
    ...
    ASSIGN_OR_RETURN(
        ExprNodePtr to,
        CallOpReference("core.to_optional",
                        {CallOpReference("core.where", {c, a, b})}));
    ASSIGN_OR_RETURN(
        optimizations.emplace_back(),
        CreatePatternBasedOptimization(from, to, {{"c", IsOptionalLikeNode}}));
  }
...
absl::Status PresenceAndOrOptimizations(
  ...
  {  // _presence_and_or(P.a, P.b, P.c)  -> P.a | P.c (if b is always present)
    ...
    ASSIGN_OR_RETURN(
        optimizations.emplace_back(),
        CreatePatternBasedOptimization(from, to, {{"b", IsAlwaysPresent}}));
  }
